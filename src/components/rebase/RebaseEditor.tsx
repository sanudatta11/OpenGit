  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={handleCancel}>
      ...
      >
        ...
        {/* Summary bar */}
        <div className="px-4 py-2 border-b border-border bg-bg-panel/40 text-xs text-fg-muted">
          ...
          {items.map((item, index) => (
            ...
            >
                  ...
                  {actionPopoverId === item.id && (
                    ...
                    >
                      {ACTIONS.map((a) => (
                        <button
                          key={a}
                          className={`text-xs text-left px-2 py-1 rounded hover:bg-bg-hover capitalize ${actionColor(a)} ${item.action === a ? 'font-semibold bg-bg-hover' : ''}`}
              ...
              {item.action === 'squash' && index > 0 && (
                <div className="px-3 pb-2 pl-[72px] text-xs text-git-modified">
